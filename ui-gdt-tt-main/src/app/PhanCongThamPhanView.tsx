...
interface DonRecord {
  ...
  hinhThucPC?: "ngau-nhien" | "chi-dinh";
}
...
export function PhanCongThamPhanView() {
  const [activeTab, setActiveTab] = useState<"ngau-nhien" | "chi-dinh" | "ket-qua">("ngau-nhien");
  ...
  const handleBulkAssign = () => {
    const idsToAssign = activeTab === "ngau-nhien"
      ? currentRecords.map((r) => r.id)
      : selectedRowIds;

    if (idsToAssign.length === 0) {
      alert(activeTab === "ngau-nhien" ? "Không có đơn đủ điều kiện để phân công!" : "Vui lòng chọn ít nhất 1 đơn để phân công!");
      return;
    }
    ...
    setRecords((prev) => prev.map((r) => {
      if (!idsToAssign.includes(r.id)) return r;
      const assignedJudge = activeTab === "chi-dinh"
        ? bulkJudge
        : DANH_SACH_THAM_PHAN[Math.floor(Math.random() * DANH_SACH_THAM_PHAN.length)];
      return {
        ...r,
        thamPhan: assignedJudge,
        ngayPhanCong: "23/06/2026",
        trangThai: "da-phan-cong",
        hinhThucPC: activeTab === "ngau-nhien" ? "ngau-nhien" : "chi-dinh",
      };
    }));

    setSelectedRowIds([]);
    alert(`Đã phân công thành công ${idsToAssign.length} đơn${activeTab === "ngau-nhien" ? " theo hình thức ngẫu nhiên" : ` cho Thẩm phán ${bulkJudge}`}!`);
    setActiveTab("ket-qua");
  };

  const handleDeleteSelected = () => {
    if (activeTab !== "ket-qua") return;
    if (selectedRowIds.length === 0) {
      alert("Chưa chọn kết quả phân công cần xóa!");
      return;
    }
    if (confirm(`Bạn có chắc chắn muốn hủy ${selectedRowIds.length} kết quả phân công?`)) {
      setRecords((prev) => prev.map((r) => selectedRowIds.includes(r.id)
        ? {
            ...r,
            trangThai: r.hinhThucPC === "ngau-nhien" ? "chua-ngau-nhien" : "chua-chi-dinh",
            thamPhan: "-",
            ngayPhanCong: "-",
          }
        : r));
      ...
      <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
          <div style={{ fontSize: 13, color: MUTED, fontFamily: F }}>
            {activeTab === "ngau-nhien" ? `Danh sách đủ điều kiện: ${currentRecords.length} đơn` : `Đã chọn ${selectedRowIds.length} mục`}
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            {activeTab === "chi-dinh" ? (
              ...
              >
                Phân công ngẫu nhiên ({currentRecords.length} đơn)
              </button>
            ) : null}
