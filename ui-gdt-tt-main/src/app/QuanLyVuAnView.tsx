...
export interface VuAnRow {
  ...
  // CR28/8: 1 vụ có thể vừa Án tử hình vừa Án người chưa thành niên,... — hiển thị đủ tag, không chỉ 1
  anLoaiTags?: string[];
  loaiAn?: string;
  ...
  diaChiNDD?: string;
  extraTags?: string[];
  // CR28/8: tội danh chính của bị cáo được khiếu nại (hoặc bị cáo đầu vụ nếu không có)
  toiDanhChinh?: string;
}
...
export default function QuanLyVuAnView({
  ...
  // CR28/8: cảnh báo số lượng vụ tại tab Đang giải quyết (và Đã giải quyết, cho đồng bộ)
  const countDangGiaiQuyet = roleGroups.filter((g) => g.rows.some((r) => r.kqGiaiQuyet === "chua-co" || r.kqGiaiQuyet === "da-co-con-don" || !r.kqGiaiQuyet)).length;
  const countDaGiaiQuyet = roleGroups.filter((g) => g.rows.some((r) => r.kqGiaiQuyet === "da-co")).length;
  const tabs = [
    { id: "tat-ca", label: "Tất cả", count: undefined as number | undefined },
          ...
          {tabs.map((t) => {
            ...
            return (
              <button key={t.id} onClick={() => setActiveTab(t.id as VuAnTabId)}
                style={{ display: "flex", alignItems: "center", gap: 6, padding: "10px 16px", fontSize: 13, fontFamily: F, fontWeight: active ? 600 : 400, background: "none", border: "none", cursor: "pointer", color: active ? RED : MUTED, borderBottom: active ? `2px solid ${RED}` : "2px solid transparent", marginBottom: -1, whiteSpace: "nowrap" }}>
                {t.label}
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
          ...
          <thead>
            <tr>
              ...
              {["STT", "Mã đơn", "Thông tin giải quyết đơn", "Ngày nhận đơn", "Họ và tên", "Phân loại", "Nội dung", "Thao tác"].map((h) => (
      ...
      <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 1000, display: "flex", alignItems: "flex-start", justifyContent: "center", overflowY: "auto", padding: "24px 16px" }}>
        <div style={{ background: "#fff", borderRadius: 10, width: "100%", maxWidth: 940, boxShadow: "0 20px 60px rgba(0,0,0,0.2)", marginBottom: 24 }}>
          ...
          <div style={{ padding: "16px 20px", overflowY: "auto" }}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ maxWidth: 300 }}>
                ...
                <select value={loaiPhieu} onChange={e => { setLoaiPhieu(e.target.value); setCoThayDoiPhieu(true); }} style={selSt}>
                  ...
                  <option value="Quyết định rút hồ sơ">Quyết định rút hồ sơ</option>
            ...
            {["Quyết định rút hồ sơ", "Phiếu trả", "Phiếu chuyển", "Công văn XM, BS", "Công văn khác"].includes(loaiPhieu) && (
            <div style={{ marginBottom: 16 }}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ background: "#fff", borderRadius: 8, border: `1px solid ${BORDER}`, overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
            ...
            <thead>
              <tr>
                {["STT", "HỌ VÀ TÊN THẨM PHÁN", "NGÀY PHÂN CÔNG", "NGƯỜI BÚT PHÊ", "GHI CHÚ"].map(h => (
                  <th key={h} style={TH_STYLE}>{h}</th>
                ))}
              </tr>
            </thead>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ background: "#fff", borderRadius: 8, border: `1px solid ${BORDER}`, overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
            ...
            <thead>
              <tr>
                {["STT", "HỌ VÀ TÊN TTV", "NGÀY PHÂN CÔNG TTV", "HỌ VÀ TÊN LÃNH ĐẠO", "NGÀY PHÂN CÔNG LĐ", "NGƯỜI PHÊ DUYỆT"].map(h => (
                  <th key={h} style={TH_STYLE}>{h}</th>
                ))}
              </tr>
            </thead>
